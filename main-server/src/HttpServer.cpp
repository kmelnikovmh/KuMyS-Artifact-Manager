...
main_server::HttpServer::HttpServer(const std::string &url,
    ...
    : listener(url), input_queue_(input_queue), output_queue_(output_queue) {

//   listener.support(methods::GET, [this](const http_request &httpRequest) {
//   handle_get_request(httpRequest); });
  listener.support(methods::POST, [this](const http_request &httpRequest) {
    handle_post_request(httpRequest);
  });
}
...
void main_server::HttpServer::start() {
  try {
    listener.open().wait();
    std::cout << "HTTP Server started at: " << listener.uri().to_string()
              << std::endl;
...
void main_server::HttpServer::stop() { listener.close().wait(); }

// TODO
void main_server::HttpServer::handle_get_request(
    const web::http::http_request &request) {}

void main_server::HttpServer::handle_post_request(
    const web::http::http_request &request) {
  request.extract_json().then(
      [this, request](const pplx::task<json::value> &task) {
        try {
...
bool main_server::HttpServer::validate_light_json(
    const main_server::LightJSON &json) {
  const std::vector<std::reference_wrapper<const std::string>> fields = {
      json.id,      json.request_type, json.name,
      json.version, json.architecture, json.check_sum};
  return std::all_of(fields.begin(), fields.end(),
                     [](const std::string &fields) { return !fields.empty(); });
}
